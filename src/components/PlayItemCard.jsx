 ...
 */
function PlayItemCard({ index, item, onChange, onToggleNote, onRemove, showGhostNotes = false }) {
    ...
    return (
        <div className="play-item-card" style={{ borderColor: color }}>
            ...
            {item.type === 'scale' ? (
                <ScaleContent item={item} onChange={onChange} onToggleNote={onToggleNote} color={color} showGhostNotes={showGhostNotes} />
            ) : (
...
function ScaleContent({ item, onChange, onToggleNote, showGhostNotes = false }) {
    const root = item.root || 'A';
    const scale = item.scale || 'major-pentatonic';
